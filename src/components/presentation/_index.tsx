import { presentationConfig } from '@/config/presentation-config'

import { Text } from '../text/_index'
import {
  PresentationInfo,
  PresentationInfoItem,
  PresentationName,
  PresentationStyled,
} from './presentation-styled'

export const Presentation = () => {
  return presentationConfig.map((config, index) => (
    <PresentationStyled key={index}>
      <PresentationName>
        <Text as="p" variant="text" size="sm">
          {config.presentation.welcome}
        </Text>
        <Text as="h1" variant="title" size="xl">
          {config.presentation.name}
        </Text>
        <Text as="h2" variant="subtitle" size="lg">
          {config.presentation.stack}
        </Text>
      </PresentationName>
      {config.infos.map((info, index) => (
        <PresentationInfo key={index} href={info.href} target="_blank">
          <Text as="p" variant="comment" size="md">
            {info.comment}
          </Text>
          <PresentationInfoItem>
            <Text as="p" variant="tag" size="md">
              const
            </Text>
            <Text as="p" variant="variable" size="md">
              {info.variable}
            </Text>
            <Text as="p" variant="text" size="md">
              = {info.value}
            </Text>
          </PresentationInfoItem>
        </PresentationInfo>
      ))}
    </PresentationStyled>
  ))
}
